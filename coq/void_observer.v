...
(* NOTE: the old Admitted in void_arithmetic.v has been proved away —       *)
(* spur_nonneg is structural (Fin has no negative inhabitants).            *)
(* But the blind spot survives: no_complete_self_model below.              *)
(******************************************************************************)
...
(* REMOVED: finite_recurrence                                               *)
(* Was declared as pigeonhole ("more steps than states → must revisit")     *)
(* but the formal statement was disconnected: n_steps and n_states were     *)
(* not linked to the trajectory. The axiom said "list is non-empty"         *)
(* with irrelevant hypotheses. Zero uses in the codebase.                   *)
(* If pigeonhole is needed later, it should be stated and proved properly.  *)
...
(******************************************************************************)
(*  EPILOG — On the Source of Budget                                         *)
(******************************************************************************)
(*                                                                           *)
...
(*                                                                           *)
(*  NOTE: The old Admitted in void_arithmetic.v — once read as a            *)
(*  Gödelian performance — has been proved away. spur_nonneg turned out    *)
(*  to be a property of the type, not of the operation. The blind spot     *)
(*  did not vanish; it migrated to no_complete_self_model (Section III):   *)
(*  a system cannot fully model itself within its own budget.               *)
(*  Gödel lives there now, as a theorem — not an Admitted.                 *)
(*                                                                           *)
(*  We start with what we have and count every tick until it runs out.      *)
(*                                                                           *)

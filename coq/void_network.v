(******************************************************************************)
(* void_network.v - LAYERS AND PROPAGATION                                   *)
(*                                                                          *)
(* Builds networks from threshold cells (void_learning_cell.v).             *)
(* Signal between layers is list Bool3 — never raw FinProb.                 *)
(* BUnknown inputs are holes: free to skip, no budget spent.                *)
(*                                                                          *)
(* WeightedCell = LearningCell + VoidVector weights.                        *)
(* Layer = list WeightedCell. Propagation: same input, independent cells.   *)
(*                                                                          *)
(* Verdict and learning mechanics live in void_neural_theory.v.             *)
(* Accumulator cells (void_predictive_cell.v) can use the same Signal       *)
(* type and Verdict, but need their own layer propagation.                  *)
(*                                                                          *)
